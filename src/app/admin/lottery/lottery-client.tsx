...
export interface LotteryClientProps {
  /** League owners, including departed ones (kept for history). */
  owners: Owner[];
  /**
   * The verified 1-12 order. Slots 1-4 lock in reverse; 5-12 seed the lottery.
   * The server only renders this component once it has a gap-free 1-12.
   */
  defaultOrder: LotterySlot[];
  /** Season the order came from — scopes localStorage so orders can't leak across years. */
  seasonYear: number;
  /** Where defaultOrder came from, shown to the admin so the source is never a guess. */
  orderSource: string;
}
...
export default function LotteryClient({
  ...
  const LOTTERY_ORDER_STORAGE_KEY = lotteryStorageKey(seasonYear);
  const [standings, setStandings] = useState<LotterySlot[]>(defaultOrder);
  ...
  // Restore a hand-entered order from a previous visit
  useEffect(() => {
    try {
      const saved = localStorage.getItem(LOTTERY_ORDER_STORAGE_KEY);
      if (!saved) return;
      const parsed: unknown = JSON.parse(saved);
      if (isValidLotteryOrder(parsed, defaultOrder)) {
        setStandings([...parsed].sort((a, b) => a.finish - b.finish));
      }
    } catch {
  ...
  const setSlotOwner = useCallback(
    (finish: number, ownerId: string) => {
      const next = applySlotSwap(standings, finish, ownerId);
      if (!next) return;

      localStorage.setItem(LOTTERY_ORDER_STORAGE_KEY, JSON.stringify(next));
      setStandings(next);
    },
    [standings, LOTTERY_ORDER_STORAGE_KEY]
  );

  const resetLotteryOrder = useCallback(() => {
    localStorage.removeItem(LOTTERY_ORDER_STORAGE_KEY);
    setStandings(defaultOrder);
  }, [defaultOrder, LOTTERY_ORDER_STORAGE_KEY]);

  // Only owners already in slots 5-12 are selectable, so a swap can never
  // pull someone out of the locked top 4.
  const lotteryOwnerIds = useMemo(
    () =>
      standings
        .filter((s) => s.finish > 4)
  ...
  const isCustomOrder = useMemo(
    () =>
      standings.some((s) => {
        const original = defaultOrder.find((d) => d.finish === s.finish);
        return original?.ownerId !== s.ownerId;
      }),
    [standings, defaultOrder]
  );
  ...
  // Debounced so rapid edits to the order don't stack 50k-sim runs.
  useEffect(() => {
    const id = setTimeout(() => {
          ...
          <div className="text-center space-y-6 fade-in">
            ...
            <div className="text-muted text-sm">
              ...
              <span className="text-muted/40 ml-2">
                Slot {currentPickNumber?.finish} — {currentPickNumber?.weight}% odds
              </span>
            </div>
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
              ...
              {Object.entries(LOTTERY_WEIGHTS)
                ...
                .map(([finish, weight]) => (
                  <div key={finish} className="flex justify-between">
                    <span className="text-muted">Slot {finish}</span>
      ...
      {/* Lottery order — slots 5-12 are hand-entered */}
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between gap-4">
          <div>
            <h2 className="font-bold text-sm">2026 Lottery Order</h2>
            <p className="text-[11px] text-muted mt-0.5">
              Slots 5-12 seed the lottery — override any of them below. Source:{" "}
              <span className="text-muted/80">{orderSource}</span>
              {isCustomOrder && (
          ...
          {isCustomOrder && (
            <button
              onClick={resetLotteryOrder}
              className="text-[10px] text-muted hover:text-accent transition-colors whitespace-nowrap"
            >
              ↺ Reset to default
            </button>
          )}
                ...
                >
                  ...
                  <td className="px-4 py-2 text-sm">
                    {isLocked ? (
                      ...
                      <select
                        value={s.ownerId}
                        onChange={(e) => setSlotOwner(s.finish, e.target.value)}
                        aria-label={`Owner in lottery slot ${s.finish}`}
                        className="w-full bg-background border border-border rounded px-2 py-1 text-sm hover:border-accent/50 focus:border-accent focus:outline-none transition-colors"
                      >
                        {lotteryOwnerIds.map((id) => (
                          <option key={id} value={id}>
                            {ownerMap.get(id)?.name}
                          </option>
                        ))}
                      </select>
                    )}
                  ...
                  return (
                    <tr key={s.ownerId} className="border-b border-border/20">
                      <td className="px-3 py-2">
                        ...
                        <div className="text-[10px] text-muted">
                          Slot {s.finish} — {LOTTERY_WEIGHTS[s.finish]}%
                        </div>
                      </td>
                      {[1, 2, 3, 4, 5, 6, 7, 8].map((pick) => {
